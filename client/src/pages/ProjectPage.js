...
const ProjectPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchProjects = async () => {
      try {
          const endpoint = username ? `http://localhost:5000/projects?username=${username}` : 'http://localhost:5000/projects';
  ...
  const handleCreate = async (newProjectData) => {
    try {
      const response = await axios.post('http://localhost:5000/projects', newProjectData, {
        headers: {
  ...
  const handleUpdate = async (id, updatedProjectData) => {
    try {
      const response = await axios.patch(`http://localhost:5000/projects/${id}`, updatedProjectData, {
        headers: {
          Authorization: `Bearer ${token}`
        }
      });
